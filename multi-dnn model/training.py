...
def _build_model(
    args: argparse.Namespace, case: CaseConfig, arch: Sequence[int]
) -> nn.Module:
    if args.model == "paper-table":
        return DNN(arch, state_dim=case.state_dim)
    input_dim = case.scalar_input_dim if args.contingency == "scalar" else case.onehot_input_dim
    return TwoStageDNN(
        input_dim=input_dim,
...
def _fit_model(
    ...
    model = _build_model(args, case, arch).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr or case.lr)
    criterion = PIMALoss(
        case, y_scaler, physics, device,
        config.cy, config.cv, config.cp, config.cq, config.rho,
    )
    validation_history = []
...
def train(args: argparse.Namespace) -> None:
    ...
    candidates = args.loss_candidate or [
        LossConfig(args.cy, args.cv, args.cp, args.cq, args.rho)
    ]
    tuning_results = []
    candidate_choices = []
    ...
    checkpoint_path = out_dir / f"{case.name}_{args.model}_{args.contingency}.pt"
    save_checkpoint(
