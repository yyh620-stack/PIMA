...
class PartitionPIMALoss:
    ...
    def __init__(
        ...
        self.cp, self.cq, self.rho = config.cp, config.cq, config.rho
...
def _build_partition_model(
    args: argparse.Namespace, spec: PartitionSpec, arch: Sequence[int]
) -> nn.Module:
    if args.model == "paper-table":
        return DNN(arch, state_dim=spec.state_dim)
    return TwoStageDNN(
        input_dim=arch[0],
...
def _fit_partition(
    ...
    model = _build_partition_model(args, spec, arch).to(device)
...
def train_partitioned(args: argparse.Namespace) -> None:
    ...
    for spec in specs:
        ...
        checkpoint_path = out_dir / (
            f"{case.name}_cluster_{spec.number:02d}_{args.model}_{args.contingency}.pt"
        )
        torch.save(
            {
