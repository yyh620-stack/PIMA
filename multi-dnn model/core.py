"""Model, configuration, normalization, and loss definitions for PIMA-COPF."""
    ...
    def parse(cls, value: str) -> "LossConfig":
        try:
            ...
            raise argparse.ArgumentTypeError(
                "loss candidate must contain numeric cy,cv,cp,cq,rho"
            ) from exc
        if len(numbers) != 5:
            raise argparse.ArgumentTypeError(
                "loss candidate must be cy,cv,cp,cq,rho"
            )
        return cls(*numbers)
...
class PIMALoss:
    ...
    def __init__(
        ...
        self.case = case
        self.cy, self.cv, self.cp, self.cq, self.rho = cy, cv, cp, cq, rho
    ...
    def update_lambdas(
        ...
        for lagrange, violation in zip(self.lambdas, violations):
            lagrange.add_(self.rho * batch_fraction * violation.mean(dim=0))
...
class DNN(nn.Module):
    """Fully connected DNN using the layer widths from Table III."""

    def __init__(self, layer_sizes: Sequence[int], state_dim: int) -> None:
        super().__init__()
        ...
        for i, width in enumerate(layer_sizes[1:], start=0):
            if width == state_dim:
                self.state_layer = i
                break
