...
r = solve_bus_affine_g;/  
r = solve_bus_affine_g(radius=0.15, w_pg=1, w_vm=100);设置波动范围15%，w是优化目标中的权重，不影响可行域，只影响多种可行解中哪一种被选中  
与论文的实现还存在以下区别：  
...
4.当前目标函数是“选取稳定仿射控制策略”的加权 L1 正则项，不是运行/再调度成本最小化等  

**cluster_buses.m**对118个母线进行K-means聚类，  
result = cluster_buses/
result = cluster_buses(k=6)指定组数为6/  
result = cluster_buses(k=0, k_range=2:15, replicates=100);应用肘部法则确定组数  
