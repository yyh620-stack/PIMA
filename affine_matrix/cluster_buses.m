...
%CLUSTER_PIMA_BUSES Cluster buses from the affine state-response matrix.
%
%   result = cluster_pima_buses;
%   result = cluster_pima_buses(k=6);
%   result = cluster_pima_buses(k=0, k_range=2:15, replicates=100);
%
% k=0 selects K by the elbow of log within-cluster sum of squares. The
% function requires MATLAB's Statistics and Machine Learning Toolbox.
...
% K-means labels are arbitrary. Reorder them by the smallest bus ID in
% each cluster so repeated runs have human-comparable cluster numbers.
...
if o.make_plots
    ...
    subplot(2,1,1);
    plot(kCandidates,wcss,'o-','LineWidth',1.4);
    hold on;
    plot(selectedK,wcss(selectedPosition),'rp','MarkerSize',12, ...
        'MarkerFaceColor','r');
    grid on;
    xlabel('Number of clusters K'); ylabel('Within-cluster sum of squares');
    title(sprintf('K selection: K = %d (%s)',selectedK,selectionMethod));

    subplot(2,1,2);
    plot(kCandidates,meanSilhouette,'o-','LineWidth',1.4);
    hold on;
    plot(selectedK,meanSilhouette(selectedPosition),'rp','MarkerSize',12, ...
        'MarkerFaceColor','r');
    grid on;
    xlabel('Number of clusters K'); ylabel('Mean silhouette');
    saveas(fig,fullfile(outputDir,'k_selection.png'));
    close(fig);

    Xcentered = X-mean(X,1);
    [U,S,~] = svd(Xcentered,'econ');
    pcaScore = U(:,1:2)*S(1:2,1:2);
    fig = figure('Visible','off','Color','w');
    scatter(pcaScore(:,1),pcaScore(:,2),42,labels,'filled');
    hold on;
    text(pcaScore(:,1),pcaScore(:,2),compose(' %g',busIds), ...
        'FontSize',7,'VerticalAlignment','middle');
    grid on; colormap(lines(selectedK)); colorbar;
    xlabel('PCA score 1'); ylabel('PCA score 2');
    title(sprintf('Affine-response bus clusters (K = %d)',selectedK));
    saveas(fig,fullfile(outputDir,'cluster_pca.png'));
    close(fig);
end
